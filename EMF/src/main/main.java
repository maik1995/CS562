...
//		loader.closeConnection();
		...
		HashMap<String,String> columnNameAndType = FindColumnType.columnNameAndType(db, "sales");
		System.out.print(columnNameAndType.toString());
		List<String> SQLquery = ReadFile.readByLine("SQL.txt");
		List<List<String>> all_order = Translate_SQL.translate(SQLquery);
		...
		System.out.println(Arrays.toString(grouping_variable));
		...
		try {
		    ...
		    writer = new FileWriter(className+".java", false);
		    	...
		    	if(entry.getKey()!="O") {
			    	...
			    	for(String aggregate_result : entry.getValue().split(",")) {
			    		...
			    		if(!m.group(4).equals("*")) 
			    	        ...
			    	        writer.write("\t\tpublic List<java.lang.Integer> count_x = new ArrayList<>();\n");
			    	}
			    	writer.write("\t"+"}"+"\n");
		    	}
		    }
            ...
            for(Map.Entry<String, String> entry: select_attribute.entrySet()) {
            	...
            	vect.put(entry.getKey(), new ArrayList<java.lang.String>());
          ...
          //creating mfs-structure
	    		...
	    		if(m.find()) {
	    			if(!m.group(4).equals("*")) {
	    				writer.write("\tmfs.put(\"" + entry.getKey() + "_" + entry.getValue().get(i) + "\", new ArrayList<" + columnNameAndType.get(m.group(4)) + ">());\n");
	    					...
	    					writer.write("\tmfs.put(\"" + entry.getKey() + "_" + entry.getValue().get(i) + "\", new ArrayList<java.lang.Integer>());\n");
	    				}
	    		}else {
            ...
            writer.write("\t\tString usr =\""+user+"\";"+"\n");
            writer.write("\t\tString pwd =\""+passWord+"\";"+"\n");
            				...
            				for(int j = 0; j < vect.get(key).size(); j++) {
                                ...
                                for(Map.Entry<String, List> entry: vect.entrySet()) {
                                	for(int i = 0; i < entry.getValue().size(); i++) {
                                		...
                                		writer.write("\t\t\t\t" + entry.getKey() + "_" + entry.getValue().get(i) + "= (java.lang.Integer)mfs.get(\"" + entry.getKey() + "_" + entry.getValue().get(i) + "\").get(i); \n");
                                		writer.write("\t\t\t }\n");
                                	}
                                }
		    ...
		    for(Map.Entry<String, List> entry: vect.entrySet()) {
	    		...
	    		if(m.find()) {
			    	...
			    	writer.write("\tresult.put(\"" + entry.getKey() + "_" + entry.getValue().get(i) + "\", new ArrayList<" + columnNameAndType.get(m.group(4)) + ">());\n");
	    				...
	    				writer.write("\tresult.put(\"" + entry.getKey() + "_" + entry.getValue().get(i) + "\", new ArrayList<java.lang.Integer>());\n");
	    			}
	    		}else {
		    ...
		    for(Map.Entry<String, List> entry: vect.entrySet()) {
	    		...
	    		if(m.find()) {
	    			if(!m.group(4).equals("*")) {
			    	writer.write("\t\tjava.lang.Integer" + " " + entry.getKey() + "_" + entry.getValue().get(i) +"=("+"\t\tjava.lang.Integer"+")"+"mfs.get(\""+ entry.getKey() + "_" + entry.getValue().get(i) + "\").get(i)"+ ";\n");
	    			}else {
	    				writer.write("\t\tjava.lang.Integer"+ " " + entry.getKey() + "_" + entry.getValue().get(i)+"=("+"java.lang.Integer"+")"+"mfs.get(\""+ entry.getKey() + "_" + entry.getValue().get(i) + "\").get(i)"+" ;\n");
	    			}
	    		}else {
		    ...
		    writer.write("\t\t\t\t\t}\n");
		    writer.write("\t\t\t\t\tCell cell = row.createCell(cellnum++);\n");
		    writer.write("\t\t\t\t\tif(data.get(j).get(i) instanceof java.lang.String)\n");
		    writer.write("\t\t\t\t\t\tcell.setCellValue((String)data.get(j).get(i));\n");
		    writer.write("\t\t\t\t\telse if(data.get(j).get(i) instanceof java.lang.Integer)\n");
